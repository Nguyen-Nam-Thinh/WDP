              ...
              <div key={race._id} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-md dark:border-slate-700 dark:bg-[#243045]">
                ...
                <div className="flex flex-col gap-2 mb-6">
                  ...
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-200">
                    💰 Giải thưởng: <span className="text-emerald-600 dark:text-emerald-400">${race.purse?.toLocaleString()}</span>
                  </p>
                </div>
        ...
        {loadingDetail ? (
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-center dark:border-slate-700 dark:bg-slate-800/50">
                <p className="text-2xl font-bold text-amber-500 dark:text-amber-400 mb-1">${totalBetAmount.toLocaleString()}</p>
                <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Tổng Tiền Cược</p>
              </div>
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-center dark:border-slate-700 dark:bg-slate-800/50">
                <p className="text-2xl font-bold text-emerald-500 dark:text-emerald-400 mb-1">${totalPayout.toLocaleString()}</p>
                    ...
                    <tbody>
                      {raceBets.map(bet => (
                        <tr key={bet._id} className="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                          ...
                          <td className="py-2.5 px-4 font-semibold text-black dark:text-white">${bet.amount.toLocaleString()}</td>
                          ...
                          <td className="py-2.5 px-4">
                            {bet.status === 'won' ? (
                              <span className="font-bold text-emerald-600 dark:text-emerald-400">${bet.payoutAmount?.toLocaleString()}</span>
                            ) : (
